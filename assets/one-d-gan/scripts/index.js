...
async function main() {
  const tf = require("tf");
  const { data, OneDGAN } = require("gan");
  const ui = require("ui");
  const { loadSVGs, wait } = require("utils");
  ...
  async function init(running, firstCall) {
    ...
    const updateGANView = ui.initGANViewUI(
      rangeInputSync,
      gan.G.predict(rangeInputs).dataSync()
    );

    const updateDBox = ui.initDBoxUI(
      rangeInputSync,
      gan.D.predict(rangeInputs).dataSync()
    );
    const updateLoss = ui.initLossUI();

    const updateGANOutputs = await ui.initGANOutputUI(
      fakeOutputsSync,
      targetDataSync
    );
