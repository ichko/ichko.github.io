...
1. Invert the `encoder` and the `decoder`, as they would be in an ordinary `AE` and try to reconstruct
   images from the MNIST dataset. Bare in mind the network has never seen natural images.

2. We will use the representations from the `image encoder` to squeeze the images from MNIST
