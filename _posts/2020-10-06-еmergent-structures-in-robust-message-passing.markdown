...
from the example above we can see that all images from the same class are "reconstructed" in
with similar shapes, which is expected.
...
multiple times with this image as the result.
...
This lead us to believe that the generator has not learned to "useful" structures,
only simple ones that have emerged from the training for robustness and the initial
parameters of the network.

With this observation we can sadly conclude that the network has NOT learned anything "useful"
...
In the next post we will experiment with `AE` as augmentation function. But for now we could not show that
