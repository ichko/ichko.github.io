...
varied in _time_ leading to smooth transitions in time.
...
class FFN(nn.Module):
    def __init__(self, width, depth):
        ...
        self.net = nn.Sequential(*[dense(
            2 if i == 0 else width,
            3 if i == depth - 1 else width,
            nn.Sigmoid if i == depth - 1 else nn.Tanh
...
```python
net = FFN(width=25, depth=9)
output = net(inp.reshape(-1, 2)).detach().numpy()
